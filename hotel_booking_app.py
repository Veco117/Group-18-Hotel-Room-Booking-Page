...
# Try to import Pillow library (required for rounded corners and color
# processing)
try:
    ...
    HAS_PIL = False
    print("Warning: Pillow library not detected. "
          "Please run: pip install pillow")
    print("Without Pillow, rounded corner "
          "effects will degrade to rectangles.")
...
from manage_booking_flow import (
    ViewBookingPage,
    ModifyBookingPage,
    CancelBookingPage
)
    ...
    try:
        with open(DB_FILE, 'r') as f:
            ...
            for booking in bookings:
                ln_match = booking["last_name"].lower() == last_name.lower()
                code_match = booking["confirmation_code"] == code
                if ln_match and code_match:
                    return booking
        return None
    except Exception:
        return None
...
def create_rounded_bg(width, height, bg_color, border_color=None, radius=25):
    """
    Draw rounded background (returns RGB mode to ensure rounded corners
    display correctly)
    """
    ...
    # Draw in RGBA mode first, then convert to RGB to ensure smooth
    # rounded edges
    ...
    if hasattr(draw, 'rounded_rectangle'):
        ...
        if border_rgb:
            ...
            # Then draw inner background (white rounded rect), leave 2px border
            draw.rounded_rectangle((2, 2, width - 3, height - 3),
                                   radius=max(radius - 2, 1), fill=bg_rgb)
        else:
            # No border, draw rounded rectangle directly
            draw.rounded_rectangle((0, 0, width - 1, height - 1),
                                   radius=radius, fill=bg_rgb)
    else:
        # If Pillow version is too old, use ellipse and rectangle combination
        # Ensure radius doesn't exceed half of width and height
        radius = min(radius, width // 2, height // 2)

        if border_rgb:
            # Draw border rectangle (middle part) first
            draw.rectangle((radius, 0, width - radius - 1, height - 1),
                           fill=border_rgb)
            draw.rectangle((0, radius, width - 1, height - radius - 1),
                           fill=border_rgb)
            # Draw four rounded corners (using ellipses)
            draw.ellipse((0, 0, radius * 2, radius * 2), fill=border_rgb)
            draw.ellipse((width - radius * 2 - 1, 0, width - 1, radius * 2),
                         fill=border_rgb)
            draw.ellipse((0, height - radius * 2 - 1, radius * 2, height - 1),
                         fill=border_rgb)
            draw.ellipse((width - radius * 2 - 1, height - radius * 2 - 1,
                          width - 1, height - 1), fill=border_rgb)

            # Then draw inner background (white rounded rect), leave 2px border
            inner_radius = max(radius - 2, 1)
            draw.rectangle((inner_radius + 2, 2, width - inner_radius - 3,
                            height - 3), fill=bg_rgb)
            draw.rectangle((2, inner_radius + 2, width - 3,
                            height - inner_radius - 3), fill=bg_rgb)
            # Draw four inner rounded corners
            draw.ellipse((2, 2, inner_radius * 2 + 2, inner_radius * 2 + 2),
                         fill=bg_rgb)
            draw.ellipse((width - inner_radius * 2 - 3, 2, width - 3,
                          inner_radius * 2 + 2), fill=bg_rgb)
            draw.ellipse((2, height - inner_radius * 2 - 3,
                          inner_radius * 2 + 2, height - 3), fill=bg_rgb)
            draw.ellipse((width - inner_radius * 2 - 3,
                          height - inner_radius * 2 - 3, width - 3, height - 3),
                         fill=bg_rgb)
        else:
            # No border, draw rounded rectangle directly
...
def create_complete_button_image(width, height, bg_color, border_color,
                                 icon_img, text_label, text_color, radius=25):
    """
    ...
    if icon_img and icon_img.size[0] > 0 and icon_img.size[1] > 0:
        ...
        icon_h_target = int(height * 0.6)  # Icon height is 60% of button ht
        ...
        icon_resized = icon_img.resize((icon_w_target, icon_h_target),
...
def combine_icon(bg_img, icon_img):
    ...
    # Resize icon to fit button
    icon_h_target = int(bg_img.height * 0.6)  # Icon height is 60% of btn ht
    if icon_img.height > 0:
        ratio = icon_h_target / icon_img.height
        icon_w_target = int(icon_img.width * ratio)
    else:
...
class CanvasButton:
    ...
    """
    def __init__(self, canvas, x, y, width, height, image_filename, variable,
                 value, text_label, mode="radio", command=None, radius=25):
        ...
        icon_size = int(height * 0.64)  # Dynamically adjust based on height
        ...
        if HAS_PIL:
            if not image_filename.startswith("icon/"):
                icon_path = f"icon/{image_filename}"
            else:
            ...
            try:
                ...
                self._icon_images = [self.icon_normal_img,
                                     self.icon_inverted_img]
    ...
    def draw_button(self, selected=False):
        ...
        # Clear previous content (reset IDs to avoid binding to deleted elems)
        if self.bg_rect_id:
        ...
        if icon_img:
            ...
            icon_y = self.y
            self.icon_id = self.canvas.create_image(icon_x, icon_y,
                                                    image=icon_img,
                                                    anchor="center",
                                                    tags=button_tag)
            ...
            text_x = icon_x + icon_size // 2 + icon_spacing  # Text left edge
            self.text_id = self.canvas.create_text(
                text_x, self.y,
                text=self.text_label,
                font=("Arial", font_size, "bold"),
                fill=text_color,
                anchor="w",  # Left align, ensure text starts from fixed pos
                tags=button_tag
            )
        else:
        ...
        self.canvas.tag_bind(button_tag, "<Button-1>",
                             lambda e: self.on_click())
        self.canvas.tag_bind(button_tag, "<Enter>",
                             lambda e: self.canvas.config(cursor="hand2"))
        self.canvas.tag_bind(button_tag, "<Leave>",
    ...
    def update_appearance(self):
        ...
        is_selected = False
        if self.mode == "radio":
            is_selected = (self.variable.get() == self.value and
                           self.variable.get() != "")
        elif self.mode == "check":
...
class FilterPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.bg_img = load_simple_image("filter_bg.png", WINDOW_WIDTH,
                                        WINDOW_HEIGHT)
        if self.bg_img:
            self.canvas.create_image(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2,
                                     image=self.bg_img, anchor="center")

        # 2. Create components directly on Canvas, not using large Frame
        ...
        self.canvas.create_text(start_x, start_y + 30, text="Your Choices",
                                font=("Arial", 32, "bold"),
                                fill="#001540", anchor="center")

        self.buttons = []

        # === Column 1: Room Type (Radio) ===
        col1_x = start_x - col_width
        # Title position: about 50 pixels above first button (consistent with
        # PRICE RANGE and input box distance)
        self.canvas.create_text(col1_x, start_y + 50 + offset_y,
                                text="ROOM TYPE",
                                font=FONT_SECTION,
                                fill="#001540",
                                anchor="center")

        # Room Type changed to multi-select mode (independent BooleanVars)
        ...
        self.canvas.create_text(col2_x, start_y + 50 + offset_y,
                                text="PREFERENCES",
                                font=FONT_SECTION,
                                fill="#001540", anchor="center")
        ...
        btn_high = CanvasButton(
            self.canvas,
            col2_x,
            start_y + 100 + offset_y + row_height,
            btn_width, btn_height,
            "icon_high.png",
            self.floor_var,
            "High",
        ...
        # Pet & Smoking (Check)
        btn_pet = CanvasButton(
            self.canvas,
            col2_x,
            start_y + 100 + offset_y + 2 * row_height,
            btn_width, btn_height,
            "icon_pet.png",
            self.pref_pet,
            True,
        ...
        btn_smoke = CanvasButton(
            self.canvas,
            col2_x,
            start_y + 100 + offset_y + 3 * row_height,
            btn_width, btn_height,
            "icon_smoking.png",
            self.pref_smoke,
            True,
        ...
        # Title position: about 50 pixels above first button
        self.canvas.create_text(col3_x, start_y + 50 + offset_y,
        ...
        # --- Price Range ---
        self.canvas.create_text(col3_x,
                                start_y + 100 + offset_y + 2 * row_height + 20,
                                text="PRICE RANGE",
                                font=FONT_SECTION,
                                fill="#001540",
                                anchor="center")

        # Use rounded rectangle background
        # (same appearance as buttons, aligned with SMOKING button)
        ...
        # (white background, blends with rounded rectangle background)
        price_container = tk.Frame(self.canvas,
                                   bg="white",
                                   highlightthickness=0)
        self.canvas.create_window(col3_x, price_y,
                                  window=price_container,
                                  width=price_w - 20,
                                  height=price_h - 10)

        # Create input boxes (white background, no border)
        entry_frame = tk.Frame(price_container,
                               bg="white",
                               highlightthickness=0)
        entry_frame.pack(expand=True, fill="both")

        # Adjust font size based on button height
        entry_font_size = max(9, int(btn_height * 0.2))
        ...
        nav_y = WINDOW_HEIGHT - 50  # Translate downward, 50 px from bottom
        ...
        # Calculate button positions (two buttons centered with spacing)
        ...
        self.canvas.tag_bind(back_bg_tag, "<Button-1>",
                             lambda e: self.on_back_to_main())
        self.canvas.tag_bind(back_bg_tag, "<Enter>",
                             lambda e: self.canvas.config(cursor="hand2"))
        self.canvas.tag_bind(back_bg_tag, "<Leave>",
                             lambda e: self.canvas.config(cursor=""))
        ...
        self.canvas.tag_bind(search_bg_tag, "<Button-1>",
                             lambda e: self.on_search())
        self.canvas.tag_bind(search_bg_tag, "<Enter>",
                             lambda e: self.canvas.config(cursor="hand2"))
        self.canvas.tag_bind(search_bg_tag, "<Leave>",
                             lambda e: self.canvas.config(cursor=""))
...
class WelcomePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.bg_img = load_simple_image("welcome_bg.png", WINDOW_WIDTH,
                                        WINDOW_HEIGHT)
        if self.bg_img:
            self.canvas.create_image(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2,
                                     image=self.bg_img, anchor="center")
        ...
        tk.Label(nav_bar, text="TVXK HOTEL", font=("Arial", 16, "bold"),
                 bg="white").pack(side="left", padx=30)
        nav_frame = tk.Frame(nav_bar, bg="white")
        nav_frame.pack(side="right", padx=30)
        for t, p in [("Rooms", "RoomsPage"), ("Location", "LocationPage"),
                     ("About us", "AboutUsPage")]:
            tk.Button(
                nav_frame, text=t, font=("Arial", 11, "bold"), bg="white",
                relief="flat", cursor="hand2",
                command=lambda p=p: controller.show_frame(p)
            ).pack(side="left", padx=15)

        # Buttons
        btn_book = tk.Button(
            self.canvas, text="BOOK NOW", font=("Arial", 12, "bold"),
            bg=SECONDARY_COLOR, fg="black", width=30, height=2, relief="flat",
            cursor="hand2",
            command=lambda: controller.show_frame("DateSelectionPage")
        )
        self.canvas.create_window(WINDOW_WIDTH // 2 - 180, WINDOW_HEIGHT - 80,
                                  window=btn_book, anchor="center")

        btn_manage = tk.Button(
            self.canvas, text="MANAGE BOOKING", font=("Arial", 12, "bold"),
            bg=SECONDARY_COLOR, fg="black", width=30, height=2, relief="flat",
            cursor="hand2",
            command=lambda: controller.show_frame("ManageBookingPage")
        )
        self.canvas.create_window(WINDOW_WIDTH // 2 + 180, WINDOW_HEIGHT - 80,
                                  window=btn_manage, anchor="center")
...
class ManageBookingPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.bg_img = load_simple_image("booking_bg.png", WINDOW_WIDTH,
                                        WINDOW_HEIGHT)
        if self.bg_img:
            self.canvas.create_image(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2,
                                     image=self.bg_img, anchor="center")

        # 3. Right side white rounded card (top doesn't exceed main area)
        ...
        # 5. Create content container on card (adjust position to move content
        # upward, remove border occlusion)
        card_container = tk.Frame(self.canvas, bg="white",
        ...
        window_id = self.canvas.create_window(
            card_x + card_width // 2,
            card_y + card_height // 2 + container_y_offset,
            window=card_container,
            width=card_width - 70,
        ...
        title_label = tk.Label(card_container, text="Manage Your Booking",
                               ...
                               fg="black")
        title_label.pack(pady=(2, 8), anchor="w")  # Top padding 2px

        # 7. Instruction text (reduce spacing to 1/2)
        instruction_label = tk.Label(
            card_container,
            text="Please enter the name and reservation number as they appear "
                 "on your reservation confirmation.",
            font=("Arial", 9), bg="white", fg="#666666",
            wraplength=card_width - 90, justify="left"
        )
        ...
        firstname_label = tk.Label(input_frame, text="First Name",
                                   font=("Arial", 10, "bold"), bg="white",
                                   fg="black", anchor="w")
        firstname_label.pack(fill="x", pady=(0, 2))
        self.entry_firstname = tk.Entry(input_frame, font=("Arial", 10),
                                        bg="#F5F5F5", relief="flat", bd=2,
                                        highlightthickness=0,
                                        insertbackground="black")
        self.entry_firstname.pack(fill="x", pady=(0, 4), ipady=2)

        # Last Name (reduce spacing and padding to 1/2, further thin border)
        lastname_label = tk.Label(input_frame, text="Last Name",
                                  font=("Arial", 10, "bold"), bg="white",
                                  fg="black", anchor="w")
        lastname_label.pack(fill="x", pady=(0, 2))
        self.entry_lastname = tk.Entry(input_frame, font=("Arial", 10),
                                       bg="#F5F5F5", relief="flat", bd=2,
                                       highlightthickness=0,
                                       insertbackground="black")
        self.entry_lastname.pack(fill="x", pady=(0, 4), ipady=2)

        # Booking Number (reduce spacing and padding to 1/2, thin border)
        booking_label = tk.Label(input_frame, text="Booking Number",
                                 font=("Arial", 10, "bold"), bg="white",
                                 fg="black", anchor="w")
        booking_label.pack(fill="x", pady=(0, 2))
        self.entry_code = tk.Entry(input_frame, font=("Arial", 10),
                                   bg="#F5F5F5", relief="flat", bd=2,
                                   highlightthickness=0,
                                   insertbackground="black")
        self.entry_code.pack(fill="x", pady=(0, 0), ipady=2)

        # 9. Result label (for displaying search results, reduce spacing)
        self.result_label = tk.Label(card_container, text="",
                                     font=("Arial", 11), bg="white",
                                     fg="green")
        self.result_label.pack(pady=(3, 0))

        # 10. Button container (ensure buttons are inside card, not occluded)
        ...
        # Log in button Canvas (black bg, white text, placed above Back button)
        login_canvas = tk.Canvas(button_frame, bg="white", width=btn_w,
                                 height=btn_h, highlightthickness=0)
        ...
        login_canvas.create_text(login_btn_x, login_btn_y, text="Log in",
                                 ...
                                 anchor="center", tags=login_btn_tag)
        login_canvas.tag_bind(login_btn_tag, "<Button-1>",
                              lambda e: self.perform_search())
        login_canvas.tag_bind(login_btn_tag, "<Enter>",
                              lambda e: login_canvas.config(cursor="hand2"))
        login_canvas.tag_bind(login_btn_tag, "<Leave>",
                              lambda e: login_canvas.config(cursor=""))

        # Or separator line (in the middle between Log in and Back buttons)
        separator_frame = tk.Frame(button_frame, bg="white", height=16)
        separator_frame.pack(fill="x", pady=(2, 2))
        separator_canvas = tk.Canvas(separator_frame, bg="white", width=btn_w,
                                     height=16, highlightthickness=0)
        separator_canvas.pack(fill="x")
        separator_line_x1 = 35
        separator_line_x2 = btn_w - 35
        separator_canvas.create_line(separator_line_x1, 8, separator_line_x2,
                                     8, fill="#E0E0E0", width=0.5)
        ...
        # Back to main menu button Canvas (light blue-green bg, black text)
        back_canvas = tk.Canvas(button_frame, bg="white", width=btn_w,
                                height=btn_h, highlightthickness=0)
        ...
        back_canvas.create_text(back_btn_x, back_btn_y,
                                text="Back to main menu",
                                font=("Arial", 11, "bold"), fill="black",
                                anchor="center", tags=back_btn_tag)
        back_canvas.tag_bind(back_btn_tag, "<Button-1>",
                             lambda e: controller.show_frame("WelcomePage"))
        back_canvas.tag_bind(back_btn_tag, "<Enter>",
                             lambda e: back_canvas.config(cursor="hand2"))
        back_canvas.tag_bind(back_btn_tag, "<Leave>",
                             lambda e: back_canvas.config(cursor=""))
    ...
    def perform_search(self):
        ...
        if booking:
            self.result_label.config(
                text=f"Found: {booking.get('room_type', 'N/A')} Room",
                fg="green"
            )
...
class ImagePageBase(tk.Frame):
    def __init__(self, parent, controller, img_file, title):
        ...
        nav_bar.place(x=0, y=0, width=WINDOW_WIDTH, height=60)
        tk.Label(nav_bar, text="TVXK HOTEL", font=("Arial", 16, "bold"),
                 bg="white", fg="black").pack(side="left", padx=30)
        nav_frame = tk.Frame(nav_bar, bg="white")
        nav_frame.pack(side="right", padx=30)
        for t, p in [("Rooms", "RoomsPage"), ("Location", "LocationPage"),
                     ("About us", "AboutUsPage")]:
        ...
        tk.Button(
            ...
            relief="flat", bg="white", fg="black", cursor="hand2"
        ).pack(side="left", padx=20)
        tk.Label(bar, text=title, bg="white", fg="black",
    ...
    def load(self, event=None):
        if not hasattr(self, 'tk_img'):
            ...
            self.tk_img = load_simple_image(self.img_file, WINDOW_WIDTH,
                                            WINDOW_HEIGHT)
            if self.tk_img:
                self.canvas.create_image(WINDOW_WIDTH // 2,
                                         WINDOW_HEIGHT // 2,
                                         image=self.tk_img,
                                         anchor="center")
